...
def upgrade():
    ...
    # Create revenue_state_totals table
    op.execute("""
        CREATE TABLE revenue_state_totals (
    ...
    op.execute("""
        CREATE TRIGGER trigger_update_revenue_state_totals_timestamp
        BEFORE UPDATE ON revenue_state_totals
        FOR EACH ROW EXECUTE FUNCTION update_date_updated_column()
    """)
    
    # Create index
    op.execute("""
        CREATE INDEX idx_revenue_state_totals_type ON revenue_state_totals(revenue_entry_type_id_fk)
    """)
    
    # Populate revenue_state_totals with data
    op.execute("""
        INSERT INTO revenue_state_totals (revenue_entry_type_id_fk, year, value)
        SELECT
            ret.id, 
            df.year, 
            SUM(r.value)
...
def downgrade():
    ...
    # Drop the revenue_state_totals table
    op.execute("DROP TABLE IF EXISTS revenue_state_totals CASCADE")
