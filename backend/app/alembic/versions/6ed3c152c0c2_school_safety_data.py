"""School Safety Data Migration

Revision ID: 6ed3c152c0c2
Revises: 99baf2d34ef7
Create Date: 2025-04-10 19:19:30.724964
...
revision = '6ed3c152c0c2'
down_revision = '99baf2d34ef7'
branch_labels = None
depends_on = None
...
def load_config():
    ...
    config_path = os.path.join(current_dir, '../config/generate_school_safety_data.yaml')
...
def get_schools() -> List[Tuple[str, int]]:
    """Get a list of all schools from the database using alembic op."""
    conn = op.get_bind()
    result = conn.execute(sa.text("SELECT id, name FROM school ORDER BY id;"))
...
def execute_sql_statements(sql_statements, batch_size=500):
    ...
    if batch_size > 0 and total > batch_size:
        ...
        for i in range(0, len(sql_statements), batch_size):
            ...
            try:
                # Open a transaction for each batch
                for stmt in batch:
...
def upgrade():
    """Import school safety data from Excel files."""
    logger.info("Starting school safety data migration")
    ...
    logger.info("School safety data migration completed successfully")


def downgrade():
    """
    Remove all school safety data.
    """
    logger.info("Removing all school safety data")
    
    # List of tables in reverse order (to handle foreign keys)
    tables = [
        "school_seclusion",
        "school_restraint",
        "school_harassment",
    ...
    logger.info("School safety data removal completed successfully")
