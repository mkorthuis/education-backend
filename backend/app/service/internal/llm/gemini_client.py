...
class GeminiConfig(BaseModel):
    ...
    model: str = Field("gemini-2.0-flash", description="Gemini model to use")
    ...
    def generate_text(self, prompt: str) -> GeminiResponse:
        ...
        try:
            ...
            if hasattr(response, "usage_metadata") and response.usage_metadata:
                # Convert UsageMetadata object to dictionary
                usage = {
    ...
    def chat(self, messages: List[Message]) -> GeminiResponse:
        ...
        try:
            ...
            if hasattr(last_response, "usage_metadata") and last_response.usage_metadata:
                # Convert UsageMetadata object to dictionary
                usage = {
                    "prompt_token_count": getattr(last_response.usage_metadata, "prompt_token_count", 0),
                    "candidates_token_count": getattr(last_response.usage_metadata, "candidates_token_count", 0),
                    "total_token_count": getattr(last_response.usage_metadata, "total_token_count", 0)
                }
